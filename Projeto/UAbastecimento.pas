...
begin
  ...
  //Habilita botão NOVO e Desabilita o botão SALVAR
  btNovo.Enabled := true;
...
begin
  //ABRE DATASET E COLOCA EM MODO DE INSERÇÃO
  DataSourceAbastecimentos.DataSet.Open;
  DataSourceAbastecimentos.DataSet.Insert;
  //Desabilita botão NOVO e Habilita botão SALVAR e CANCELAR
...
begin
  if DBEdtData.text = '  /  /    ' then
    begin
    ShowMessage('O Campo Data é Obrigatório');
    DBEdtData.SetFocus;
    exit;
    end;
  ...
  if DBEdtBomba.text = '' then
    begin
    ShowMessage('O Campo Bomba é Obrigatório');
    DBEdtData.SetFocus;
    exit;
    end;
  ...
  if DBEdtQtdLitros.text = '' then
    begin
    ShowMessage('O Campo Qtd.Litros é Obrigatório');
    DBEdtData.SetFocus;
    exit;
    end;
  ...
  DataModule1.ClientDataSet_ABASTECIMENTOSVALOR_LIQUIDO.Value :=  //CALCULA VALOR LÍQUIDO
  ...
  //Desabilita botão NOVO e Habilita botão SALVAR
  btNovo.Enabled := true;
  btSalvar.Enabled := false;
end;
